...
# read in results df -----------------------------------------------------------
df_all <- readRDS("/rivm/s/ainsliek/results/2022-08-19-rivm-vacamole.rds")
...
# calculate absolute and percent difference in disease outcomes -----
table1 <- for_plot2 %>%
  # sum disease outcomes over days
  group_by(scenario_id, outcome, sample) %>% # sum over age groups too
  summarise(value_cum = sum(value, na.rm = TRUE)) %>%
  ungroup() %>%
  # determine percent difference for each sample
  group_by(outcome, sample) %>%
  mutate(
    abs_diff  = value_cum - value_cum[scenario_id == 'E'],
    abs_diff = ifelse(is.nan(abs_diff), NA, abs_diff),
    perc_diff = (value_cum - value_cum[scenario_id == 'E'])/value_cum[scenario_id == 'E'],
    perc_diff = ifelse(is.nan(perc_diff), NA, perc_diff)
    ) %>%
  # calculate summary statistics
  group_by(scenario_id, outcome) %>%
  summarise(
