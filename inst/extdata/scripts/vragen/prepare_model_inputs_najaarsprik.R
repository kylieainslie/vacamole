...
# --- Skip if vac schedule has already been updated ---
...
file_name <- "Cum_upt20220714.csv"

vac_sched <- read_csv(paste0(vac_path,file_name)) %>%
...
# Create vaccination schedules for scenarios
# A = 50% coverage 60+, 50% coverage 18+
...
# Update vac schedule for Round 3 ----------------------------------------------
...
# 2) add 5th doses 
# assume 5th doses distribution is 25% pfizer and 75% moderna (up to XX coverage)
# 5th doses start 15 September and end 15 December
n_days_5d <- as.numeric(as.Date("2022-12-15") - as.Date("2022-09-15"))
vac_cov_vec_pf_5d <- seq(from = 0, to = (vac_coverage * 0.25), length.out = n_days_5d)
vac_cov_vec_mo_5d <- seq(from = 0, to = (vac_coverage * 0.75), length.out = n_days_5d)
...
#saveRDS(vac_schedule_5da, "inst/extdata/inputs/vaccination_schedules/vac_schedule_scenario_hub_round3_AC.rds")
# 18 - 59 year olds
# 1) increase coverage of 3rd dose in 18-59 year olds
vac_schedule <- vac_schedule_5da #readRDS("inst/extdata/inputs/vaccination_schedules/vac_schedule_scenario_hub_round2_AC.rds")
...
cov_dat <- data.frame(date = seq.Date(extra_start_date_4d, extra_end_date_4d, 
                      ...
                      vac_cov_d4_2 = seq(from = tail(vac_schedule_3d$pf_d4_2,1), 
                                         to = (vac_coverage2*0.2) - tail(vac_schedule_3d$mo_d4_2,1), 
                                         length.out = n_days_4d),
                      vac_cov_d4_3 = seq(from = tail(vac_schedule_3d$pf_d4_3,1), 
                                         to = vac_coverage2 - tail(vac_schedule_3d$mo_d4_3,1), 
                                         length.out = n_days_4d),
                      vac_cov_d4_4 = seq(from = tail(vac_schedule_3d$pf_d4_4,1), 
                                         to = vac_coverage2 - tail(vac_schedule_3d$mo_d4_4,1), 
                                         length.out = n_days_4d),
                      vac_cov_d4_5 = seq(from = tail(vac_schedule_3d$pf_d4_5,1), 
                                         to = vac_coverage2 - tail(vac_schedule_3d$mo_d4_5,1), 
                                         length.out = n_days_4d),
                      vac_cov_d4_6 = seq(from = tail(vac_schedule_3d$pf_d4_6,1), 
                                         to = vac_coverage2 - tail(vac_schedule_3d$mo_d4_6,1), 
                                         length.out = n_days_4d)
                        )
...
saveRDS(vac_schedule1, 
        paste0("inst/extdata/inputs/vaccination_schedules/vac_schedule_60plus_",
                              vac_coverage,"_60minus_",vac_coverage2,".rds"))
