...
stochastic_age_struct_seir_ode <- function(times,init,params){
  with(as.list(c(params,init)), {
    ...
    gamma <- gamma/dt         # 1/gamma = infectious period
    sigma <- sigma/dt         # 1/sigma = latent period
    h <- h/dt                 # Rate from infection to hospital admission/ time from infection to hosp admission
    i1 = i1/dt                # rate from hopsital to IC
    i2 = i2/dt                # rate from IC back to hospital (H_IC)
    d = d/dt                  # rate from H to D
    d_ic = d_ic/dt            # rate from IC to D
    d_hic = d_hic/dt          # rate from H_IC to D
    r = r/dt                  # rate from H to R
    r_ic = r_ic/dt            # rate from H_IC to R 
